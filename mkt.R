...
#2. Insert Values
api_key <-'T90enrBmwool2BV6RuEDjvEzM'
api_secret <- '0GAGPBBLOsKM2a4XMlj7JZkTgAaaQbIRdk9Y8YA9nYq96UtXQ5'
access_token <- '1181396196008574976-VJsoCodAdpqntefUaFLvazbbXDq5KB'
access_token_secret <-'mj5BC3A7OpHLVYFVZT0qAWxYzceULMJHcSO3ArENIYVjN'
...
#3. Extract tweets
#Take a look at Youtube tweets
#pull 1200 tweets
tweets <-searchTwitter("Youtube", n=1200, lang='en')
head(tweets)
length(tweets)
#[1] 1200
...
yt<-twListToDF(tweets)

#4. Create CSV file and save it
write.csv(yt, file = '~/Desktop/yt.csv', row.names = F)

# Data Cleaning and Preparation
#1. Reading Data File
yt<-read.csv(file.choose('youtube.csv'), header=T) #choose the yt.csv file
str(yt) #look at structure of the  file (has 1000 obs and 16 var)
...
yt$text <- sapply(yt$text,function(row) iconv(row, "latin1", "ASCII", sub=""))

#2. Building Corpus
library(tm)
library(NLP)
corpus <-iconv(yt$text, to='utf-8-mac') #need only the first col text from file
...
#tweets were pulled using youtube so we can clean it from the text
cleanset <-tm_map(cleanset, removeWords, c('youtube','utube','just','get','lets','will'))
inspect(cleanset[1:5])
...
barplot(w, las = 2, col=rainbow(40),main ='Word Frequency for Youtube Tweets') #words represented vertically using las=2, rainbow colors 
...
#Reading Files
#take the initial apple tweet file 
#take the first column, text and put it into tweets dataframe
tweets <- iconv(yt$text, to="utf-8-mac")
...
barplot(colSums(s), 
        las = 2,
        ylab = 'Total Count', 
        main ='Sentiment Scores for Youtube Tweets',col=rainbow(30))
...
termM[1:10, 1:10] #term term matrix, 
